...
module FrontApi
  class MergeContacts
    ...
    attr_accessor :target_contact_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FrontApi::MergeContacts` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FrontApi::MergeContacts`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'target_contact_id')
        self.target_contact_id = attributes[:'target_contact_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          target_contact_id == o.target_contact_id &&
    ...
    def hash
      [target_contact_id, contact_ids].hash
    end
