  ...
  class RemoveContactsFromGroup
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FrontApi::RemoveContactsFromGroup` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FrontApi::RemoveContactsFromGroup`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @contact_ids.length > 50
        invalid_properties.push('invalid value for "contact_ids", number of items must be less than or equal to 50.')
      end
    ...
    def valid?
      ...
      return false if @contact_ids.length > 50
      true
    end
    ...
    def contact_ids=(contact_ids)
      ...
      if contact_ids.length > 50
        fail ArgumentError, 'invalid value for "contact_ids", number of items must be less than or equal to 50.'
      end
